 public class spearman_rho_JW
    {//Abre la clase que contiene a los métodos para cálculo del coeficiente relacional de Spearman o Rho
     ...
     sum = 0;//Inicializa acumulador en cero
      ...
      sum = sum/n;//Promedia los valores acumulados
       ...
       for(j=0;j<m;j++){//Abre ciclo for de j que recorre todos los valores hasta m
          ...
          if(emp_acum[j]!=0&&emp_acum[j+1]==0||emp_acum[j]!=0&&j==m-1){//Abre if simple 
               calc_rangos(j,emp_acum[j],n,2,rangos_temp_y,datos_rango_y);//Invoca a función que calcula rangos y modifica el arreglo de rangos temporal
                                                                     }//Cierra if simple                 
             }//Cierra try
