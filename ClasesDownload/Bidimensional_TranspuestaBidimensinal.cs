...
{//Abre la clase que contiene a los métodos de cálculo de la matríz transpuesta bidimensional
           ...
           int i,j; //subíndices de renglones y columnas
             for(i=0;i<ren;i++) //Inicia ciclo for de los renglones
               for(j=0;j<col;j++){  //Inicia ciclo for de las columnas
                   A_transpuesta[j,i] = A_entrada[i,j];//Transpone cada elemento del arreglo
